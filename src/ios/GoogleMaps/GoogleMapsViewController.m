...
- (void) mapView:(GMSMapView *)mapView willMove:(BOOL)gesture
{
  ...
  // use the _onMapEvent callback instead of the _onCameraEvent callback.
	NSString* jsString = [NSString
    stringWithFormat:@"javascript:cordova.fireDocumentEvent('%@', {evtName: '%@', callback: '_onMapEvent', args: []});",
                        self.mapId, gesture ? @"map_drag_start" : @"camera_move_start"];
...
- (void)mapView:(GMSMapView *)mapView didChangeCameraPosition:(GMSCameraPosition *)position {
  [self triggerCameraEvent:(self.isDragging ? @"map_drag" : @"camera_move") position:position];
}
...
{
  [self triggerCameraEvent:(self.isDragging ? @"map_drag_end" : @"camera_move_end") position:position];
  self.isDragging = NO;
}
